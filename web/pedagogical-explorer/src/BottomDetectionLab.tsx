...
const copy = {
  en: { title:'Bottom Detection', module:'ACQUISITION CHAIN', chain:'INPUT → DETECTION → TIMING', intro:'Change the detector inputs and watch the selected echo move on the matched-filter trace.', back:'System Map', lang:'PT-BR', reset:'Reset', scenario:'Echo scenario', clean:'Single clear echo', late:'Later echo', double:'Competing echoes', delay:'TX delay', steering:'Steering angle', method:'Detection method', amp:'Amplitude peak', phase:'Phase zero crossing', window:'Detection window', windowStart:'Window start', windowEnd:'Window end', windowHint:'Only echoes inside the highlighted time window can be selected.', trace:'Matched-filter magnitude', selected:'Selected detection', twtt:'TWTT', arrival:'Arrival offset', angle:'Detected angle', peakLag:'Peak lag', unsupported:'This detection method is not available yet', unsupportedDetail:'Phase zero crossing is not yet available in this lesson. Select Amplitude peak to continue exploring the detection.', intuition:'Physical intuition', insight:'The detector selects one echo candidate. That selected timing is what continues downstream toward sounding formation.', relationship:'Signal to sounding timing', transmit:'Transmit reference', echo:'Selected echo', sounding:'Timing passed downstream', noDetection:'No echo selected', error:'The detection view could not be updated. Adjust the controls or try again.' },
  pt: { title:'Detecção do Fundo', module:'CADEIA DE AQUISIÇÃO', chain:'ENTRADA → DETECÇÃO → TEMPO', intro:'Altere as entradas do detector e observe o eco selecionado se deslocar no traço do filtro casado.', back:'Mapa do Sistema', lang:'EN', reset:'Redefinir', scenario:'Cenário de eco', clean:'Um eco nítido', late:'Eco mais tardio', double:'Ecos concorrentes', delay:'Atraso de TX', steering:'Ângulo de steering', method:'Método de detecção', amp:'Pico de amplitude', phase:'Cruzamento de fase por zero', window:'Janela de detecção', windowStart:'Início da janela', windowEnd:'Fim da janela', windowHint:'Somente ecos dentro da janela de tempo destacada podem ser selecionados.', trace:'Magnitude do filtro casado', selected:'Detecção selecionada', twtt:'TWTT', arrival:'Offset de chegada', angle:'Ângulo detectado', peakLag:'Atraso do pico', unsupported:'Este método de detecção ainda não está disponível', unsupportedDetail:'O cruzamento de fase por zero ainda não está disponível nesta lição. Selecione Pico de amplitude para continuar explorando a detecção.', intuition:'Intuição física', insight:'O detector seleciona um eco candidato. Esse tempo selecionado é o que segue adiante para a formação da sondagem.', relationship:'Do sinal ao tempo da sondagem', transmit:'Referência de transmissão', echo:'Eco selecionado', sounding:'Tempo enviado adiante', noDetection:'Nenhum eco selecionado', error:'Não foi possível atualizar a visualização da detecção. Ajuste os controles ou tente novamente.' }
};
...
export default function BottomDetectionLab({onBack}:{onBack:()=>void}) {
  ...
  useEffect(()=>{
    ...
    setError(false);
    fetch('/api/v1/pedagogical/bottom-detection',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({correlation_real:scenarios[scenario],reference_sample_count:2,sample_rate_hz:40000,tx_delay_ms:txDelay,steering_across_track_angle_deg:steering,detection_method:method,detection_window_start_ms:windowStartUs/1000,detection_window_end_ms:windowEndUs/1000})})
  ...
  },[scenario,txDelay,steering,method,windowStartUs,windowEndUs]);

  const reset=()=>{setScenario('clean');setTxDelay(0.2);setSteering(0);setMethod('amplitude_peak');setWindowStartUs(WINDOW_MIN_US);setWindowEndUs(WINDOW_MAX_US);};
  ...
  return <div className="d9-lab">
    ...
    <main className="d9-layout">
      ...
      <section className="d9-stage">
        ...
        {error?<div className="d9-message" role="status">{t.error}</div>:data?.status==='unsupported'?<div className="d9-message"><strong>{t.unsupported}</strong><span>{t.unsupportedDetail}</span></div>:<div className="d9-chart" aria-label={t.trace}>{data?.correlation.magnitude.map((m,i)=>{const lag=data.correlation.lag_us[i];const active=detection?.peak_index===i;const inWindow=lag>=windowStartUs&&lag<=windowEndUs;return <div key={i} className={`d9-sample ${active?'active':''}`} style={{height:`${8+82*m/maxMag}%`,opacity:inWindow?1:0.22}} aria-label={`${lag.toFixed(0)} µs${inWindow?' · '+t.window:''}`}><span>{active?'▲':''}</span></div>})}<div className="d9-axis">{data?.correlation.lag_us.map((lag,i)=><span key={i}>{i%2===0?`${lag.toFixed(0)} µs`:''}</span>)}</div></div>}
