...
function tracePath(trace:Trace|undefined,xDomain:Domain,yDomain:Domain,width=760,height=180){
 ...
 return trace.x.map((x,i)=>{const px=((x-xDomain.min)/dx)*width;const py=height-((trace.y[i]-yDomain.min)/dy)*height;return `${i?'L':'M'} ${px.toFixed(2)} ${py.toFixed(2)}`}).join(' ');
}
...
function Plot({trace,xDomain,yDomain,xTicks,yTicks,xLabel,yLabel,className='wave-path',pulseEnd}:{trace?:Trace;xDomain:Domain;yDomain:Domain;xTicks:string[];yTicks:string[];xLabel:string;yLabel:string;className?:string;pulseEnd?:number}){
 ...
 return <div className="scientific-plot clean-plot"><div className="plot-y-label">{yLabel}</div><div className="plot-y-ticks"><span>{yTicks[0]}</span><span>{yTicks[1]}</span><span>{yTicks[2]}</span></div><svg viewBox="0 0 760 180" preserveAspectRatio="none" aria-hidden="true"><path className="plot-grid" d="M0 45H760 M0 90H760 M0 135H760 M190 0V180 M380 0V180 M570 0V180"/><path className="gridline" d="M0 90H760"/><path className={className} d={d}/></svg>{pulsePosition!==null&&pulseEnd!==undefined&&pulseEnd<=xDomain.max&&<div className="pulse-duration-marker" style={{left:`${pulsePosition}%`}}><i/><span>{pulseEnd.toFixed(1)} ms</span></div>}<div className="fixed-axis">{xTicks.map(v=><span key={v}>{v}</span>)}</div><div className="plot-x-label">{xLabel}</div></div>;
}

export default function SignalLab({onBack,focus}:{onBack:()=>void;focus?:string}){
 const[locale,setLocale]=useState<Locale>('en');const[config,setConfig]=useState<SignalConfig>(initial);const[data,setData]=useState<SignalResponse|null>(null);const[loading,setLoading]=useState(true);const[error,setError]=useState<string|null>(null);const[requestVersion,setRequestVersion]=useState(0);const t=text[locale];
 useEffect(()=>{const controller=new AbortController();const timer=window.setTimeout(async()=>{setLoading(true);setError(null);try{const response=await fetch(`${API_BASE}/api/v1/pedagogical/signal`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(config),signal:controller.signal});if(!response.ok)throw new Error(`HTTP ${response.status}`);setData(await response.json() as SignalResponse)}catch(err){if(!controller.signal.aborted)setError(err instanceof Error?err.message:String(err))}finally{if(!controller.signal.aborted)setLoading(false)}},90);return()=>{window.clearTimeout(timer);controller.abort()}},[config,requestVersion]);
 const patch=(next:Partial<SignalConfig>)=>setConfig(current=>({...current,...next}));
 const active=focus==='spectrum'?'frequency':focus==='pulse'?'overview':focus??'overview';
 const focusTitle=active==='frequency'?t.focusFrequency:active==='compression'?t.focusCompression:active==='waveform'?t.focusWaveform:t.overview;
 const showWave=active==='overview'||active==='waveform';const showFrequency=active==='overview'||active==='frequency';const showCompression=active==='overview'||active==='compression';
 return <div className={`signal-lab signal-focus-${active}`}>
  ...
  <div className="lab-layout"><aside className="control-surface"><div className="control-title"><SlidersHorizontal size={17}/><strong>{t.controls}</strong></div>
   ...
   <button className="reset" onClick={()=>setConfig(initial)}><RotateCcw size={15}/> {t.reset}</button>{loading&&<div className="scientific-status">{t.loading}</div>}{error&&<div className="scientific-error"><strong>{t.offline}</strong><button onClick={()=>setRequestVersion(v=>v+1)}>{t.retry}</button></div>}
  </aside><section className="visual-chain signal-continuous clean-signal-plots">
   {showWave&&<div className="chain-card transmit"><header><div><strong>{t.passband}</strong></div><span>{config.pulse_type==='lfm'?'LFM':'CW'}</span></header><Plot trace={data?.waveform} xDomain={TIME_DOMAIN} yDomain={WAVE_DOMAIN} pulseEnd={config.duration_ms} xTicks={['0','2.5','5']} yTicks={['+1','0','−1']} xLabel={t.timeAxis} yLabel={t.amplitudeAxis}/></div>}
   {showFrequency&&<div className="chain-card receive"><header><div><strong>{t.instant}</strong></div><span>{config.pulse_type==='lfm'?config.chirp_direction.toUpperCase():'CW'}</span></header><Plot trace={data?.instantaneous_frequency} xDomain={TIME_DOMAIN} yDomain={FREQUENCY_DOMAIN} pulseEnd={config.duration_ms} className="echo-path" xTicks={['0','2.5','5']} yTicks={['850','425','0']} xLabel={t.timeAxis} yLabel={t.frequencyAxis}/></div>}
   {showCompression&&<div className="chain-card compression"><header><div><strong>{t.matched}</strong></div></header><Plot trace={data?.matched_filter} xDomain={MATCHED_LAG_DOMAIN} yDomain={MATCHED_DOMAIN} xTicks={['−5000','0','+5000']} yTicks={['+1','0','−1']} xLabel={t.lagAxis} yLabel={t.amplitudeAxis}/></div>}
  </section></div>
 </div>;
}
