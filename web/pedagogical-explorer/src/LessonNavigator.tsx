...
import './lesson-navigator-polish.css';
...
const copy={
 en:{lesson:'LAB',previous:'Previous',next:'Next',labs:'Labs',availableLabs:'Available labs',close:'Close labs',home:'Home',acousticLab:'Acoustic Lab',lang:'PT-BR'},
 pt:{lesson:'LAB',previous:'Anterior',next:'Próximo',labs:'Labs',availableLabs:'Labs disponíveis',close:'Fechar labs',home:'Home',acousticLab:'Laboratório de Acústica',lang:'EN'}
};
...
export default function LessonNavigator({currentId}:{currentId:string}){
 ...
 return <><div className={`lesson-shell family-${current.family}`}>
  ...
  <button className="lesson-menu-button lesson-language-button" onClick={()=>setLanguage(lang==='en'?'pt':'en')} aria-label={lang==='en'?'Mudar idioma para português':'Switch language to English'}><span>{t.lang}</span></button>
 ...
 {open&&<div className="lesson-drawer-backdrop" onClick={()=>setOpen(false)}><aside className="lesson-drawer" aria-label={t.availableLabs} onClick={e=>e.stopPropagation()}><header><div><small>{t.acousticLab.toUpperCase()}</small><strong>{t.labs}</strong></div><button onClick={()=>setOpen(false)} aria-label={t.close}><X size={18}/></button></header><div className="lesson-drawer-list">{lessons.map(l=><button key={l.id} aria-current={l.id===current.id?'page':undefined} className={`family-${l.family} ${l.id===current.id?'active':''}`} onClick={()=>go(l)}><span>{l.displayId}</span><div><strong>{lessonTitle(l,lang)}</strong></div><ArrowRight size={14}/></button>)}</div></aside></div>}
 </>
}
