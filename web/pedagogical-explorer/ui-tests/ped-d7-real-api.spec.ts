...
test('D7 real API: geometry and directional response follow learner controls', async ({ page }) => {
  ...
  page.on('response', async response => {
    ...
    if (response.url().endsWith(directionalPath) && response.status() === 200) {
      directionalResponses.push(await response.json());
    }
  });
  ...
  // Geometry contract: changing Sector must cross the real HTTP boundary and
  // change the canonical angular extent sent to the Python API.
  ...
  // Directional contract: TX across-track beamwidth changes the directional
  // model itself. Text labels are intentionally not used as evidence because
  // they need not change when the plotted response changes.
  const directionalPanel = page.getByTestId('d7-selected-directional');
  await expect(directionalPanel).toBeVisible();
  ...
  expect(directionalRequests.at(-1).transmit_across_track_beamwidth_deg)
    .not.toBe(directionalRequests.at(-2).transmit_across_track_beamwidth_deg);
