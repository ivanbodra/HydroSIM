...
test('D6 keeps the array fixed and exposes authoritative channel timing plus shared RX virtual beams',async({page})=>{
 ...
 await expect(page.getByText('One RX snapshot → several virtual beams',{exact:true})).toBeVisible();
 await expect(page.locator('.shared-rx>div')).toHaveCount(6);
 ...
 expect(requests.at(-1)?.steering_control_mode).toBe('delay_gradient');
