...
const endpoint = '/api/v1/pedagogical/bottom-detection';
...
test('D8 real API: gate threshold and High Density state cross the boundary', async ({ page }) => {
  ...
  const thresholdBefore = Number(await threshold.inputValue());
  const thresholdCount = requests.length;
  await threshold.press('ArrowRight');
  await expect.poll(() => requests.length).toBeGreaterThan(thresholdCount);
  expect(requests.at(-1).threshold).not.toBe(thresholdBefore);
  expect(statuses.at(-1)).toBe(200);

  const windowEnd = page.getByRole('slider', { name: 'Window end' });
  const windowBefore = Number(await windowEnd.inputValue());
  const windowCount = requests.length;
  await windowEnd.press('ArrowLeft');
  await expect.poll(() => requests.length).toBeGreaterThan(windowCount);
  expect(requests.at(-1).detection_window_end_ms).toBeCloseTo(Number(await windowEnd.inputValue()) / 1000);
  expect(Number(await windowEnd.inputValue())).not.toBe(windowBefore);
  expect(statuses.at(-1)).toBe(200);

  const highDensity = page.getByRole('combobox', { name: 'High Density' });
  const hdCount = requests.length;
  await highDensity.selectOption('on');
  await expect.poll(() => requests.length).toBeGreaterThan(hdCount);
  expect(requests.at(-1).high_density.high_density_enabled).toBe(true);
  await expect.poll(() => statuses.length).toBeGreaterThan(3);
  expect(statuses.at(-1)).toBe(200);
