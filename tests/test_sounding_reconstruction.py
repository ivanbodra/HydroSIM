...
def test_layered_nadir_reconstruction_stops_on_measured_twtt_not_average_speed() -> None:
    ...
    assert sounding.ray_path.travel_time_seconds == pytest.approx(one_way_time)
    assert len(sounding.ray_path.segments) == 2
    assert sounding.point.x == pytest.approx(0.0)
    assert sounding.point.y == pytest.approx(0.0)
    assert sounding.point.z == pytest.approx(85.0)
    assert sounding.reconstruction_assumption == "stationary_reciprocal_layered_sound_speed_twtt_ray_trace"
