...
def test_selected_rx_follows_canonical_port_positive_beam_and_tx_stays_nadir() -> None:
    ...
    tx_peak = response.tx_one_way.angle_deg[
        response.tx_one_way.normalized_power.index(max(response.tx_one_way.normalized_power))
    ]
    rx_peak = response.rx_one_way.angle_deg[
        response.rx_one_way.normalized_power.index(max(response.rx_one_way.normalized_power))
    ]
