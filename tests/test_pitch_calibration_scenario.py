...
def test_pitch_calibration_preserves_reciprocal_labels_and_solution() -> None:
    result = run_pitch_calibration_scenario(PitchCalibrationScenarioConfig())

    assert (result.pass_a_uncorrected.heading_deg, result.pass_b_uncorrected.heading_deg) == (0.0, 180.0)
    assert (result.pass_a_corrected.heading_deg, result.pass_b_corrected.heading_deg) == (0.0, 180.0)
    assert result.estimated_pitch_alignment_deg == pytest.approx(1.0, abs=0.02)


def test_pitch_calibration_requires_identifiable_positive_slope() -> None:
