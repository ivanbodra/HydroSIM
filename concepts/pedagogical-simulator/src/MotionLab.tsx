...
export default function MotionLab({ onBack }: { onBack: () => void }) {
  ...
  const gesture = (_: PointerEvent, info: PanInfo) => {
    setRoll(Math.max(-20, Math.min(20, Math.round(info.offset.x / 9))));
    setHeave(Math.max(-35, Math.min(35, Math.round(info.offset.y / 4))));
  };
  return <div className="phenomenon-lab motion-lab">
    ...
    <div className="lab-question"><span>05 · MOTION</span><h1>Move the vessel and watch the whole measurement geometry react.</h1><p>Roll, pitch, yaw and heave become gestures. Drag the vessel itself or use the controls; the ghost platform preserves the baseline for immediate comparison.</p></div>
    <div className="phenomenon-layout">
      ...
      <section className="phenomenon-stage motion-stage"><div className="stage-kicker"><div><small>DIRECT MANIPULATION</small><strong>Baseline ghost + current platform</strong></div><span>MOVE → SEE CONSEQUENCE</span></div><div className="motion-scene">
        <div className="motion-gesture-hint"><Hand size={14}/> Drag vessel: horizontal → roll · vertical → heave</div><div className="horizon-line"/><div className="ghost-vessel"><div className="motion-hull"/><span>baseline</span></div><motion.div className={`current-vessel ${dragging?'dragging':''}`} drag dragMomentum={false} dragElastic={0} onDragStart={()=>setDragging(true)} onDrag={gesture} onDragEnd={()=>setDragging(false)} animate={{rotate:roll,y:heave,x:yaw*.8,skewX:pitch*.35}} transition={{type:'spring',stiffness:90,damping:16}}><div className="motion-hull"/><div className="motion-bridge"/><div className="motion-axis roll"><Rotate3D size={18}/><span>roll</span></div><div className="motion-axis yaw"><Compass size={18}/><span>yaw</span></div><div className="motion-beam"><i/><i/><i/><i/><i/></div></motion.div>
        <motion.div className="motion-pulse" animate={{scale:[.5,9],opacity:[.8,0]}} transition={{duration:2,repeat:Infinity}}/><div className="motion-seabed"><span/><span/><span/><span/><span/><span/></div><div className="sounding-trail">{Array.from({length:18}).map((_,i)=><motion.i key={i} animate={{x:roll*(i-9)*.28+yaw*.5,y:Math.abs(pitch)*(i%3)*.7}} transition={{type:'spring',stiffness:80,damping:18}}/> )}</div><div className="heave-ruler"><Waves size={16}/><span>heave</span><b style={{transform:`translateY(${heave}px)`}}/></div><div className="motion-readout"><small>VISUAL AMPLIFICATION</small><strong>{dragging?'You are moving the platform':'Motion is exaggerated for learning'}</strong><span>The scene prioritizes interaction language; values and geometry remain illustrative concept behavior.</span></div>
      </div><div className="causal-strip"><div><small>1 · MOVE</small><strong>Touch the platform</strong><span>Direct drag now coexists with explicit controls.</span></div><div><small>2 · COMPARE</small><strong>Keep the ghost state</strong><span>The neutral vessel never disappears.</span></div><div><small>3 · FOLLOW</small><strong>Watch beam and soundings</strong><span>Downstream visual elements react in the same scene.</span></div></div></section>
    </div>
  </div>;
}
