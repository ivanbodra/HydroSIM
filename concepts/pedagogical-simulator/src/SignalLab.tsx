...
export default function SignalLab({onBack,focus}:{onBack:()=>void;focus?:string}){
 ...
 const focusTitle=focus==='pulse'?'Pulse':focus==='spectrum'?'Frequency':focus==='compression'?'Compression':'Waveform';
 const metadata=data?.metadata;

 return <div className={`signal-lab signal-focus-${focus??'waveform'}`}>
  <div className="lab-toolbar"><button className="back-button" onClick={onBack}>{t.back}</button><div className="lab-breadcrumb"><Activity size={17}/><span>Signal</span><b>/</b><strong>{focusTitle}</strong></div><div className="lab-toolbar-actions"><span className="concept-chip"><Sparkles size={13}/> {t.ready}</span><button onClick={()=>setLocale(value=>value==='en'?'pt-BR':'en')}>{locale==='en'?'PT-BR':'EN'}</button></div></div>
  ...
  <div className="lab-layout"><aside className="control-surface"><div className="control-title"><SlidersHorizontal size={17}/><strong>{t.controls}</strong></div>
   ...
   <div className="control-readouts"><div><small>Mode</small><strong>{config.pulse_type==='lfm'?'LFM':'CW'}</strong></div><div><small>f₀</small><strong>{config.center_frequency_khz.toFixed(0)} kHz</strong></div><div><small>τ</small><strong>{config.duration_ms.toFixed(1)} ms</strong></div><div><small>B</small><strong>{config.pulse_type==='lfm'?`${config.bandwidth_khz.toFixed(0)} kHz`:'—'}</strong></div></div>
  ...
  <section className="visual-chain signal-continuous"><div className="signal-flow-label"><span>TRANSMIT</span><i/><span>FREQUENCY</span><i/><span>PROCESS</span></div>
   ...
   <div className="chain-arrow"><span>processing lens</span><i>→</i></div>
   <div className="chain-card compression"><header><div><small>{t.process}</small><strong>{t.matched}</strong></div><span>DERIVED</span></header><ScientificTrace trace={data?.matched_filter}/><div className="trace-units"><span>{data?.matched_filter.x_unit??'us'}</span><span>{data?.matched_filter.y_unit??''}</span></div><div className="insight"><Waves size={18}/><span>{t.intuitionBody}</span></div></div>
  </section></div>
  <div className="causal-strip signal-causal"><div><small>{t.cause}</small><strong>Configured</strong><span>{t.causeBody}</span></div><div><small>{t.effect}</small><strong>Derived</strong><span>{t.effectBody}</span></div><div><small>{t.intuition}</small><strong>INPUT → RESPONSE</strong><span>{t.intuitionBody}</span></div></div>
 </div>;
}
