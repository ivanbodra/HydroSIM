...
const trackIcon: Record<TrackKey, typeof GraduationCap> = {
...
export default function CurriculumMap({ onOpenLegacy }: { onOpenLegacy: () => void }) {
  ...
  const chooseTrack = (key: TrackKey) => {
    setTrack(key);
    const nextTrack = pedagogicalTracks.find((item) => item.key === key)!;
  ...
  const open = (experience: Experience) => {
    ...
    const route = productionRoute(experience);
    if (route) window.location.hash = route;
  };

  return (
    <div className={`curriculum-map track-${track}`}>
      <header className="curriculum-head">
        ...
        <button onClick={onOpenLegacy}>
          <Eye size={16} /> Original visual labs
        </button>
      </header>

      <nav className="track-switcher">
        {pedagogicalTracks.map((item) => {
          const Icon = trackIcon[item.key];
          return (
            <button
              key={item.key}
              className={track === item.key ? 'active' : ''}
              onClick={() => chooseTrack(item.key)}
            >
              <Icon size={20} />
              <span>
                <strong>{item.title}</strong>
                <small>
                  {item.experiences.length} experiences · {item.subtitle}
                </small>
              </span>
            </button>
          );
      ...
      <div className="curriculum-workspace">
        <section className="experience-list">
          <div className="track-title">
            <span>{current.key.toUpperCase()}</span>
          ...
          <div className="experience-grid">
            {current.experiences.map((experience) => (
              <motion.button
                key={experience.id}
                className={selected.id === experience.id ? 'selected' : ''}
                onClick={() => setSelected(experience)}
                whileHover={{ y: -2 }}
              >
                <span>{experience.id}</span>
                <strong>{experience.title}</strong>
                <small>{experience.outputs[0]}</small>
                <ArrowRight size={14} />
              </motion.button>
            ))}
          </div>
        ...
        <aside className="experience-inspector">
          ...
          <p className="design-cue">{selected.designCue}</p>
          <div className="io-flow">
            ...
            <Route size={20} />
            <div>
              <small>LEARNING OUTPUTS / VISUALIZATIONS</small>
              <div className="io-pills output">
          ...
          <div className="concept-loop">
            <small>CONCEPTUAL INTERACTION LOOP</small>
            <div>
              <span>INPUT</span><i>→</i><strong>MANIPULATE</strong><i>→</i><span>SEE CONSEQUENCE</span>
            </div>
          </div>
          {productionRoute(selected) ? (
            <button className="launch-experience" onClick={() => open(selected)}>
              <FlaskConical size={16} /> Open current visual foundation
            </button>
          ) : (
            <button className="launch-experience future" disabled>
              <FlaskConical size={16} /> Dedicated concept workspace next
            </button>
          )}
