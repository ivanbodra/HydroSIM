"""Deterministic P2 Pitch Patch Test reference scenario.

The implementation follows ``docs/science/pitch_patch_test_v0_1_contract.md``.
It reuses HydroSIM sounding geometry to generate reciprocal near-nadir profiles
over a planar along-track slope. The learner-facing estimate minimizes reciprocal-
profile RMS mismatch; hidden Truth is used only to generate synthetic observations
and validation diagnostics, never in the estimator objective.
"""
...
def estimate_pitch_correction(config: PitchCalibrationScenarioConfig) -> float:
    """Estimate correction using deterministic bounded coarse/fine grid refinement."""
    ...
    fine_lower = max(lower, best_coarse - coarse_step)
    fine_upper = min(upper, best_coarse + coarse_step)
