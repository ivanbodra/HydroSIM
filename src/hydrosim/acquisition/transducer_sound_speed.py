...
This keeps transducer steering separate from water-column ray tracing. Once the
physical launch/arrival direction at the transducer is established, the full SVP
controls subsequent refraction.
"""
...
class TransducerSoundSpeedSteering(BaseModel):
    """Principal-plane steering under configured and physical transducer sound speed."""
...
class TransducerSoundSpeedDirection(BaseModel):
    """Full 3-D direction implied by a configured steering law and physical local c."""
...
def _validate_sound_speeds(configured_sound_speed_mps: float, physical_sound_speed_at_transducer_mps: float) -> tuple[float, float]:
...
def resolve_transducer_sound_speed_steering(
    *,
    ...
    """Resolve the physical principal-plane angle produced by transducer steering.

    ``configured_sound_speed_mps`` is the value used to calculate the array delay
    law. ``physical_sound_speed_at_transducer_mps`` is the actual local sound speed
    at the transducer. The signed HydroSIM steering convention is preserved.
    """
    ...
    return TransducerSoundSpeedSteering(
...
def resolve_transducer_sound_speed_direction(
    *,
    configured_direction_array_frame: Vector3,
    configured_sound_speed_mps: float,
    physical_sound_speed_at_transducer_mps: float,
    ...
    return TransducerSoundSpeedDirection(
